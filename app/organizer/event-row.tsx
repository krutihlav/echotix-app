...
import { deleteEventOrganizer, toggleEventPublishedOrganizer } from './actions'
...
export default function OrganizerEventRow({ event, sold, qty }: { event: Ev; sold: number; qty: number }) {
  ...
  function onToggle() {
    ...
    startTransition(async () => {
      const res = await toggleEventPublishedOrganizer(event.id, next)
  ...
  function onDelete() {
    ...
    setError(null)
    startTransition(async () => {
      const res = await deleteEventOrganizer(event.id)
      if (res?.error) setError(res.error)
  ...
  return (
    <div className="ev-row">
      <div className="ev-row-main">
        ...
        <span className="meta">
          {[event.cat, event.date, `${sold}/${qty} prodáno`].filter(Boolean).join(' · ')}
        </span>
